<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 018 project banner" width="1200">
</p>
...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-layered-paper-cut-logic)
...
018 reverses that logic:

```text
lock three identity cues and the core relation → choose one visual anchor → translate only a few environment relations into foreground / midground / background layers → build hierarchy with fold / cut / occlusion / scale / axes / positive-negative form → assign main / dark structural / light layer / tiny accent colours → preserve warm-ivory space → build a reading path with one title + 2–4 microtext groups
```
...
- **Clear editorial hierarchy:** scale, axes, positive/negative form, front/back overlap, and generous warm-ivory space support one focus.
- **Restrained fold and cut:** folds explain joints, plane changes, direction, or depth; clear edges and soft contact shadows explain layering rather than sculptural drama.
...
- **Complete microtype:** one short title plus two to four tiny groups form a reading path along paper edges, contours, axes, and negative space; automatic copy uses no year.

## Samples · Coming soon

The repository reserves [`assets/examples/`](assets/examples/) for future work. Only finished 018 artwork verified by the project owner will be added; no post or image from another style is used as a placeholder.

Future samples will demonstrate 018's adaptability. Their subjects, metaphors, palette, copy, and canvas ratios will never become generation references or defaults.

## Four outputs, one layered paper-cut logic
...
| --- | --- | --- |
| `top-bottom` | source-adaptive | complete source above, 018 minimal layered paper-cut microtype editorial below; both panels retain the source size and split exactly 50/50 |
| `left-right` | source-adaptive | complete source left, 018 minimal layered paper-cut microtype editorial right; both panels retain the source size and split exactly 50/50 |
...
Exact user pixels > explicit ratio or destination > source adaptation for ordinary modes. The original `018.md` used a 3:4 creative canvas, but that historical example is not a silent default in the current Skill.
...
- **Independent set:** every device references only the source photograph and may explore a different anchor scale, depth-layer relation, negative space, colour-role balance, and microtype path.
...
## Paper carries visual memory; microtype carries editorial order
...
Automatic copy distils one short title from identity, known place, action, material, emotion, relation, or supported symbolism, then adds two to four tiny supporting groups.

Automatic copy never invents or uses a year; exact user wording remains verbatim even when it contains one. The title carries recognition and emotion while microtype carries order, rhythm, classification, and refinement along paper edges, contours, axes, layers, and negative space.
...
The image model creates the visual anchor, sparse depth layers, restrained folds and cuts, colour roles, matte paper, contact shadows, and complete microtype. `scripts/compose_panel.py` only plans canvases, performs exact 50/50 raster composition, finalises dimensions, and audits results. It never fakes artwork with programmatic drawing.
...
- [Chinese full prompt](references/xxd-panel-018-prompt.zh-CN.md)
- [English full prompt](references/xxd-panel-018-prompt.en.md)
- [Original style brief](references/018-source.md)
...
**A few paper layers preserve visual memory; microtype turns it into collectible order.**
